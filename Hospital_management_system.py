...
con = a.connect(host="localhost",username="root",passwd="(give your password)",database="hos")
c = con.cursor()
con.commit()
...
def patient():
    ...
    if ans == "1":
        ...
        dept_code = input("Choose a department code: ")
        dept_mapping = {
            "1": "CD",
            "2": "NU",
            "3": "DM",
        ...
        if dept_code in dept_mapping:
            dept_abbr = dept_mapping[dept_code]
            print(f"Selected department abbreviation: {dept_abbr}")  # Debug print
            c.execute(f"SELECT name FROM doc WHERE dept='{dept_abbr}'")
            result = c.fetchall()
            print(f"Fetched results: {result}")  # Debug print
            if result:
                for row in result:
                    print("Dr. " + clean(str(row[0])))
            else:
                print("No doctors found for this department.")
            con.commit()
        else:
            print("Enter a valid department code")
    elif ans == "4":
